...
const ManageOrders = () => {
  ...
  // Discount on Ready orders (Super Admin only)
  ...
  const [transferOutletStep, setTransferOutletStep] = useState(false);
  const [selectedTransferOutlet, setSelectedTransferOutlet] = useState('');
  ...
  const submitEditOrder = async () => {
    try {
      ...
      // Super admin can set/edit base_size — appears only on the kitchen view + KOT.
      const bs = editFormData.base_size;
      if (bs !== '' && bs !== null && bs !== undefined && !Number.isNaN(parseFloat(bs))) {
        updateData.base_size = parseFloat(bs);
      } else if (bs === '' || bs === null) {
        updateData.base_size = null;
      }
  ...
  const submitDiscount = async () => {
    ...
    if (amt > (discountOrder.pending_amount || 0)) {
      setMessage({ type: 'error', text: 'Discount cannot exceed pending amount' });
      return;
    }
    try {
      setDiscountSubmitting(true);
      const res = await axios.post(
        `${API_URL}/api/orders/${discountOrder.id}/apply-discount`,
        { discount_amount: amt, reason: discountReason || null },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setMessage({
        type: 'success',
        text: `Discount of ₹${amt} applied. New total: ₹${res.data.total_amount}, pending: ₹${res.data.pending_amount}`
      });
      setDiscountDialogOpen(false);
      setDiscountOrder(null);
      fetchOrders();
    } catch (error) {
      setMessage({ type: 'error', text: error.response?.data?.detail || 'Failed to apply discount' });
    } finally {
  ...
  const closeCameraDialog = () => {
    ...
    setCapturedPhoto(null);
    setDeliveryChoiceStep(false);
    setTransferOutletStep(false);
    setSelectedTransferOutlet('');
  };

  // Upload photo + transfer to specific outlet (optionally customer pickup)
  const uploadCapturedAndTransferOutlet = async (pickupByCustomer) => {
    if (!capturedPhoto?.blob || !cameraOrderId || !selectedTransferOutlet) return;
    ...
    try {
      ...
      await axios.post(
        `${API_URL}/api/orders/${cameraOrderId}/transfer-outlet?outlet_id=${encodeURIComponent(selectedTransferOutlet)}&pickup_by_customer=${pickupByCustomer}`,
        {},
        { headers: { 'Authorization': `Bearer ${token}` } }
      );
      closeCameraDialog();
      setMessage({
        type: 'success',
        text: pickupByCustomer
          ? 'Photo uploaded! Order set for customer pickup at selected outlet.'
          : 'Photo uploaded! Order transferred to selected outlet.'
      });
      fetchOrders();
      setTimeout(() => setMessage({ type: '', text: '' }), 5000);
    } catch (error) {
      console.error('Upload/transfer failed:', error);
    ...
    printWindow.document.write(`
      ...
      <html>
        ...
        <body>
          ...
          ${order.delivery_address ? `<div style="font-size:11px;">Addr: ${order.delivery_address}</div>` : ''}
          <hr class="divider" />

          <div style="font-size:12px;">CAKE DETAILS</div>
          <div class="row">
            <span>Flavour:</span>
            <span>${order.flavour || 'N/A'}</span>
          </div>
          <div class="row">
            <span>Size:</span>
            <span>${order.size_pounds} Pounds</span>
          </div>
          ${order.base_size ? `
          <div class="row">
            <span>Base Size:</span>
            <span>${order.base_size} Pounds</span>
          </div>` : ''}
          <div class="row">
            <span>Occasion:</span>
          ...
          <div class="row">
            ...
            <span>${order.name_on_cake ? '"' + order.name_on_cake + '"' : 'N/A'}</span>
          </div>
          ...
          <div class="center" style="margin-top:4px; font-size:12px; border:1.5px solid #000; padding:3px;">
  ...
  const handleBulkPrintKOT = () => {
    ...
    let html = `<!DOCTYPE html><html><head><title>Bulk KOT</title>
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-weight: bold !important; }
        body { font-family: 'Courier New', 'Lucida Console', monospace; font-size: 13px; line-height: 1.45; color: #000; font-weight: bold; }
        ...
        .instructions { border: 1.5px solid #000; padding: 5px; margin: 4px 0; white-space: pre-line; font-size: 13px; }
        .footer { font-size: 11px; text-align: center; margin-top: 6px; }
    ...
    ordersToPrint.forEach(order => {
      ...
      html += `<div class="kot-page">
        ...
        <div class="center" style="font-size:11px;">Kitchen Order Ticket</div>
        <hr class="double-divider" />
        <div class="row"><span class="big">Order #${order.order_number}</span></div>
        ...
        <hr class="divider" />
        <div style="font-size:12px;">CAKE DETAILS</div>
        <div class="row"><span>Flavour:</span><span>${order.flavour || 'N/A'}</span></div>
        <div class="row"><span>Size:</span><span>${order.size_pounds} Pounds</span></div>
        ${order.base_size ? `<div class="row"><span>Base Size:</span><span>${order.base_size} Pounds</span></div>` : ''}
        <div class="row"><span>Occasion:</span><span>${order.occasion || 'N/A'}</span></div>
        <div class="row"><span>Name on Cake:</span><span>${order.name_on_cake ? '"' + order.name_on_cake + '"' : 'N/A'}</span></div>
                                ...
                                <DropdownMenu>
                                  ...
                                  <DropdownMenuContent align="end">
                                    ...
                                    {user?.role === 'super_admin' &&
                                      (order.status === 'ready' || order.status === 'ready_to_deliver') &&
                                      (order.pending_amount || 0) > 0 && (
                                      <DropdownMenuItem
                                        onClick={() => openDiscountDialog(order)}
                                        className="text-green-700"
                                        data-testid={`apply-discount-${order.id}`}
                                      >
              ...
              <div className="space-y-4 py-4">
                ...
                {user?.role === 'super_admin' && (
                  <div className="bg-pink-50 border border-pink-200 rounded-md p-3">
                    <Label className="text-pink-700 font-semibold">Base Size (Pounds) — Admin Only</Label>
                    <Input
                      type="number"
                      step="0.5"
                      min="0"
                      placeholder="Visible only on kitchen view & KOT"
                      value={editFormData.base_size ?? ''}
                      onChange={(e) => setEditFormData({...editFormData, base_size: e.target.value})}
                      data-testid="edit-base-size-input"
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      Used by the kitchen production sheet. Hidden from the order-entry form.
                    </p>
                  </div>
                )}
                <div>
        ...
        <Dialog open={cameraDialogOpen} onOpenChange={(open) => { if (!open) closeCameraDialog(); }}>
          <DialogContent className="max-w-lg">
            ...
            <div className="space-y-4">
                  ...
                  {!capturedPhoto ? (
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 mt-3">
                        ...
                        {/* Always force the 3-way choice after capturing */}
                        <Button 
              ...
              {/* Step 2: Deliver vs Transfer vs Pickup Choice */}
              {deliveryChoiceStep && !transferOutletStep && capturedPhoto && (
                <div>
                  <img 
                    src={capturedPhoto.url} 
                    alt="Captured cake" 
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    ...
                    >
                      ...
                      {uploadingPhoto ? 'Processing...' : 'Assign Delivery'}
                    </Button>
                    <Button 
                      className="w-full bg-orange-500 hover:bg-orange-600 text-white py-6 text-base"
                      onClick={() => setTransferOutletStep(true)}
                      disabled={uploadingPhoto}
                      data-testid="choice-transfer-btn"
                    >
                    ...
                    >
                      ...
                      {uploadingPhoto ? 'Processing...' : 'Customer Pickup at Current Outlet'}
                    </Button>
                    <Button 
                      variant="outline"
                      className="w-full"
              ...
              {/* Step 3: Pick which outlet to transfer to + delivery or pickup there */}
              {deliveryChoiceStep && transferOutletStep && capturedPhoto && (
                <div className="space-y-4">
                  <img
                    src={capturedPhoto.url}
                  ...
                  />
                  <div>
                    <Label className="text-sm font-semibold">Transfer to Outlet</Label>
                    <Select value={selectedTransferOutlet} onValueChange={setSelectedTransferOutlet}>
                      <SelectTrigger data-testid="transfer-outlet-select">
                        <SelectValue placeholder="Select target outlet" />
                      </SelectTrigger>
                      <SelectContent>
                        {outlets
                          .filter(o => o.id !== cameraOrder?.outlet_id)
                          .map(o => (
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    <Button
                      className="w-full bg-cyan-600 hover:bg-cyan-700 text-white py-5"
                      onClick={() => uploadCapturedAndTransferOutlet(false)}
                      disabled={uploadingPhoto || !selectedTransferOutlet}
                      data-testid="transfer-and-deliver-btn"
                    >
                      <Truck className="h-5 w-5 mr-2" />
                      {uploadingPhoto ? 'Processing...' : 'Transfer & Deliver from There'}
                    </Button>
                    <Button
                      className="w-full bg-green-600 hover:bg-green-700 text-white py-5"
                      onClick={() => uploadCapturedAndTransferOutlet(true)}
                      disabled={uploadingPhoto || !selectedTransferOutlet}
                      data-testid="transfer-and-pickup-btn"
                    >
                      <Package className="h-5 w-5 mr-2" />
                      {uploadingPhoto ? 'Processing...' : 'Transfer & Customer Pickup There'}
                    </Button>
                    <Button variant="outline" onClick={() => { setTransferOutletStep(false); setSelectedTransferOutlet(''); }} disabled={uploadingPhoto}>
                      Back
                    </Button>
                  </div>
                </div>
