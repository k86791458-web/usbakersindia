    ...
    <LayoutWithSidebar>
                ...
                <Card key={order.id} className="hover:shadow-lg transition-shadow">
                  ...
                  <CardContent className="space-y-3">
                    ...
                    {order.order_taken_by_name && (
                      <div className="flex items-center gap-2 text-sm text-gray-700 bg-pink-50 rounded px-2 py-1" data-testid={`pending-taken-by-${order.id}`}>
                        <User className="h-4 w-4 text-pink-500" />
                        <span className="text-xs">Taken by:</span>
                        <span className="text-xs font-semibold text-pink-700">{order.order_taken_by_name}</span>
                      </div>
                    )}
      ...
      <DeleteOrderDialog
        ...
        onDeleted={(msg) => {
          ...
          fetchPendingOrders();
          alert(msg || 'Order deleted successfully!');
        }}
      />
    </LayoutWithSidebar>
  );
