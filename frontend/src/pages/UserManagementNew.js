...
const UserManagementNew = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    outlet_scope: 'specific', // 'all' | 'multiple' | 'specific'
    outlet_id: '',
    outlet_ids: [],
    permissions: [],
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2 col-span-2">
                        ...
                        <div className="flex gap-2 flex-wrap">
                          {[
                            { v: 'all', label: 'All Outlets' },
                            { v: 'multiple', label: 'Multiple Outlets' },
                            { v: 'specific', label: 'Specific Outlet' },
                          ].map(opt => (
                            <button
                              type="button"
                              key={opt.v}
                              onClick={() => setFormData({ ...formData, outlet_scope: opt.v, outlet_id: '', outlet_ids: [] })}
                              className={`px-4 py-2 rounded-full border text-sm font-medium transition-all ${
                                formData.outlet_scope === opt.v
                                  ? 'bg-pink-600 text-white border-pink-600 shadow-sm'
                                  : 'bg-white text-gray-700 border-gray-300 hover:border-pink-400'
                              }`}
                              data-testid={`outlet-scope-${opt.v}`}
                            >
                              {opt.label}
                            </button>
                          ))}
                        ...
                        {formData.outlet_scope === 'specific' && (
                          <Select
                        ...
                        {formData.outlet_scope === 'multiple' && (
                          <div className="border rounded-lg p-3 space-y-2 max-h-48 overflow-y-auto">
                            {outlets.length === 0 && (
                              <p className="text-sm text-gray-500">No outlets found. Create outlets first.</p>
                            )}
                            {outlets.map((outlet) => {
                              const checked = formData.outlet_ids.includes(outlet.id);
                              return (
                                <div key={outlet.id} className="flex items-center space-x-2">
                                  <Checkbox
                                    ...
                                    onCheckedChange={() => {
                                      const next = checked
                                        ? formData.outlet_ids.filter((i) => i !== outlet.id)
                                        : [...formData.outlet_ids, outlet.id];
                                      setFormData({ ...formData, outlet_ids: next });
                                    }}
                                    data-testid={`outlet-checkbox-${outlet.id}`}
                                  />
                                  <label htmlFor={`ol-${outlet.id}`} className="text-sm cursor-pointer">
                                    {outlet.name} <span className="text-xs text-gray-500">({outlet.city})</span>
                                  </label>
                                </div>
                              );
                            })}
                            <p className="text-xs text-gray-500 pt-2 border-t">
                              Selected: <strong>{formData.outlet_ids.length}</strong> outlet(s)
                            </p>
                          </div>
                        )}

                        {formData.outlet_scope === 'all' && (
                          <p className="text-xs text-green-700 bg-green-50 border border-green-200 rounded px-3 py-2">
                            User will have access to orders & data across <strong>all outlets</strong>.
                          </p>
                        )}
                      </div>
